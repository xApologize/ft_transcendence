...
const fragmentShader = () => {
	...
	float grid(vec2 fragCoord, float space) {
		...
		vec2 pos = refPos * 1.;
		if ( mod(fragCoord.x + space/2., space) < space * .8 && mod(fragCoord.y + space/2., space) < space * .8 )
			return clamp( 	1. - distance(pos, r) * 3., 0., 1. );
		return 0.0;
	}

	void main() {
		vec2 bufPlaneSize = vec2( 36., 20. );
		vec2 uv = v_position.xy / u_planeSize + .5;

		vec3 col = texture2D( u_frameBuffer, uv ).xyz * 0.8;
		col += vec3( grid(v_position.xy * .5, .1) );
	...
	constructor() {
		super( {
			uniforms: {
				...
				u_planeSize: { value: new Vector2( 36, 20 ) },
